...
/// comments.
public class BrowserHost : Proxy<cef_browser_host_t> {
    ...
    public static func createBrowser(windowInfo: WindowInfo,
                                     client: Client? = nil,
                                     url: NSURL? = nil,
                                     settings: BrowserSettings,
                                     requestContext: RequestContext? = nil) -> Bool {
    ...
    /// thread.
    public static func createBrowserSync(windowInfo: WindowInfo,
                                         client: Client? = nil,
                                         url: NSURL? = nil,
                                         settings: BrowserSettings,
                                         requestContext: RequestContext? = nil) -> Browser? {
        var cefSettings = settings.toCEF()
        let cefURLPtr = url != nil ? CEFStringPtrCreateFromSwiftString(url!.absoluteString) : nil
        var cefWinInfo = windowInfo.toCEF()
        let cefClient = client != nil ? client!.toCEF() : nil
    ...
    public var client: Client? {
        ...
        return ClientMarshaller.take(cefClient)
    }
    ...
    public var requestContext: RequestContext? {
        ...
        return RequestContext.fromCEF(cefCtx)
    }
    ...
    public func runFileDialog(mode: FileDialogMode,
                              title: String?,
                              defaultPath: String?,
    ...
    public func find(identifier: FindIdentifier, searchText: String, forward: Bool, caseSensitive: Bool, findNext: Bool) {
    ...
    public func showDevTools(windowInfo: WindowInfo,
                             client: Client,
                             settings: BrowserSettings,
    ...
    /// disabled.
    public func invalidate(element: PaintElementType) {
        cefObject.invalidate(cefObjectPtr, element.toCEF())
    }

    /// Send a key event to the browser.
    public func sendKeyEvent(event: KeyEvent) {
    ...
    public func sendMouseClickEvent(event: MouseEvent, type: MouseButtonType, mouseUp: Bool, clickCount: Int) {
        var cefEvent = event.toCEF()
    ...
    /// relative to the upper-left corner of the view.
    public func sendMouseMoveEvent(event: MouseEvent, mouseLeave: Bool) {
        var cefEvent = event.toCEF()
    ...
    public func sendMouseWheelEvent(event: MouseEvent, deltaX: Int, deltaY: Int) {
        var cefEvent = event.toCEF()
    ...
    public var textInputContext: TextInputContext {
        let rawHandle:UnsafeMutablePointer<Void> = cefObject.get_nstext_input_context(cefObjectPtr)
        return Unmanaged<TextInputContext>.fromOpaque(COpaquePointer(rawHandle)).takeUnretainedValue()
    }

    /// Handles a keyDown event prior to passing it through the NSTextInputClient
    /// machinery.
    public func handleKeyEventBeforeTextInputClient(event: EventHandle) {
        let rawEvent = UnsafeMutablePointer<Void>(Unmanaged<EventHandle>.passUnretained(event).toOpaque())
        cefObject.handle_key_event_before_text_input_client(cefObjectPtr, rawEvent)
    }
    
    /// Performs any additional actions after NSTextInputClient handles the event.
    public func handleKeyEventAfterTextInputClient(event: EventHandle) {
        let rawEvent = UnsafeMutablePointer<Void>(Unmanaged<EventHandle>.passUnretained(event).toOpaque())
    ...
    public func dragTargetDragEnter(dragData: DragData, event: MouseEvent, operationMask: DragOperationsMask) {
    ...
    /// This method is only used when window rendering is disabled.
    public func dragTargetDragOver(event: MouseEvent, operationMask: DragOperationsMask) {
        var cefEvent = event.toCEF()
    ...
    /// This method is only used when window rendering is disabled.
    public func dragTargetDrop(event: MouseEvent) {
        var cefEvent = event.toCEF()
    ...
    /// DragSource* mthods.
    /// This method is only used when window rendering is disabled.
    public func dragSourceEndedAt(x: Int, y: Int, operationMask: DragOperationsMask) {
    ...
    static func fromCEF(ptr: ObjectPtrType) -> BrowserHost? {
        return BrowserHost(ptr: ptr)
    }

}


public extension BrowserHost {
    /// Retrieve a snapshot of current navigation entries as values sent to the
    /// specified visitor. If |current_only| is true only the current navigation
    /// entry will be sent, otherwise all navigation entries will be sent.
    public func enumerateNavigationEntries(currentOnly: Bool, block: NavigationEntryVisitorVisitBlock) {
        enumerateNavigationEntriesUsingVisitor(NavigationEntryVisitorBridge(block: block), currentOnly: currentOnly)
    }
    ...
    public func runFileDialog(mode: FileDialogMode,
                              ...
                              acceptFilters: [String],
                              selectedFilterIndex: Int,
                              block: RunFileDialogCallbackOnFileDialogDismissedBlock) {
        runFileDialog(mode,
                      title: title,
                      defaultPath: defaultPath,
                      acceptFilters: acceptFilters,
                      selectedFilterIndex: selectedFilterIndex,
                      callback: RunFileDialogCallbackBridge(block: block))
    }
}
