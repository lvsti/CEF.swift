...
public class Request: Proxy<cef_request_t> {
    ...
    public var postData: POSTData? {
        get {
            ...
            return POSTData.fromCEF(cefPOSTDataPtr)
        }
    ...
    public func set(url: NSURL, method: String, postData: POSTData? = nil, headers: HeaderMap) {
        let cefURLPtr = CEFStringPtrCreateFromSwiftString(url.absoluteString)
        let cefMethodPtr = CEFStringPtrCreateFromSwiftString(method)
    ...
    public var flags: URLRequestFlags {
        get {
            let cefFlags = cefObject.get_flags(cefObjectPtr)
            return URLRequestFlags(rawValue: UInt32(cefFlags))
        }
    ...
    static func fromCEF(ptr: ObjectPtrType) -> Request? {
