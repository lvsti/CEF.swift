...
/// on any thread.
public class DragData: Proxy<cef_drag_data_t> {
    ...
    public func clone() -> DragData? {
        let cefData = cefObject.clone(cefObjectPtr)
        return DragData.fromCEF(cefData)
    }
    ...
    public func getFileContents(writer: StreamWriter? = nil) -> size_t {
    ...
    static func fromCEF(ptr: ObjectPtrType) -> DragData? {
        return DragData(ptr: ptr)
    }
}
