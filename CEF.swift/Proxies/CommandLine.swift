...
public class CommandLine: Proxy<cef_command_line_t> {
    
    /// Returns the singleton global CefCommandLine object. The returned object
    /// will be read-only.
    public static func globalCommandLine() -> CommandLine? {
        return CommandLine(ptr: cef_command_line_get_global())
    }
    ...
    public func copy() -> CommandLine? {
        let copiedObj = cefObject.copy(cefObjectPtr)
        return CommandLine.fromCEF(copiedObj)
    }
    ...
    static func fromCEF(ptr: ObjectPtrType) -> CommandLine? {
        return CommandLine(ptr: ptr)
    }
}
