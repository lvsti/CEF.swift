...
/// the object.
public class XMLReader: Proxy<cef_xml_reader_t> {

    /// Create a new CefXmlReader object. The returned object's methods can only
    /// be called from the thread that created the object.
    public init?(stream: StreamReader, encoding: XMLEncodingType, uri: NSURL) {
    ...
    public var type: XMLNodeType {
        ...
        return XMLNodeType.fromCEF(cefType)
    }
    ...
    static func fromCEF(ptr: ObjectPtrType) -> XMLReader? {
        return XMLReader(ptr: ptr)
    }
}
