...
public class RequestContext: Proxy<cef_request_context_t> {

    /// Returns the global context object.
    public static func globalContext() -> RequestContext? {
    ...
    /// |handler|.
    public init?(settings: RequestContextSettings, handler: RequestContextHandler? = nil) {
    ...
    /// optional |handler|.
    public static func createSharedWithContext(context: RequestContext,
                                               handler: RequestContextHandler? = nil) -> RequestContext? {
        var cefHandlerPtr: UnsafeMutablePointer<cef_request_context_handler_t> = nil
        if let handler = handler {
            cefHandlerPtr = handler.toCEF()
        }
    ...
    /// object.
    public func isSameAs(other: RequestContext) -> Bool {
        return cefObject.is_same(cefObjectPtr, other.toCEF()) != 0
    }

    /// Returns true if this object is sharing the same storage as |that| object.
    public func isSharingStorageWithContext(other: RequestContext) -> Bool {
    ...
    public var handler: RequestContextHandler? {
        let cefHandler = cefObject.get_handler(cefObjectPtr)
        return RequestContextHandlerMarshaller.take(cefHandler)
    }
    ...
    public var defaultCookieManager: CookieManager? {
    ...
    public func getDefaultCookieManagerWithCallback(callback: CompletionCallback? = nil) -> CookieManager? {
    ...
    static func fromCEF(ptr: ObjectPtrType) -> RequestContext? {
        return RequestContext(ptr: ptr)
    }
}
...
public extension RequestContext {
    ...
    /// |callback| is non-NULL it will be executed asnychronously on the IO thread
    /// after the manager's storage has been initialized.
    public func defaultCookieManager(block: CompletionCallbackOnCompleteBlock) -> CookieManager? {
        return getDefaultCookieManagerWithCallback(CompletionCallbackBridge(block: block))
    }
