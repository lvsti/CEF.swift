...
public class BinaryValue: Proxy<cef_binary_value_t> {
    ...
    /// data.
    public func isSameAs(other: BinaryValue) -> Bool {
        return cefObject.is_same(cefObjectPtr, other.toCEF()) != 0
    }
    
    /// Returns true if this object and |that| object have an equivalent underlying
    /// value but are not necessarily the same object.
    public func isEqualTo(other: BinaryValue) -> Bool {
        return cefObject.is_equal(cefObjectPtr, other.toCEF()) != 0
    }
    
    /// Returns a copy of this object. The data in this object will also be copied.
    public func copy() -> BinaryValue? {
        let copiedObj = cefObject.copy(cefObjectPtr)
        return BinaryValue.fromCEF(copiedObj)
    }
    ...
    static func fromCEF(ptr: ObjectPtrType) -> BinaryValue? {
        return BinaryValue(ptr: ptr)
    }
}
