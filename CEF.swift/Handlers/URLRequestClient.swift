...
public protocol URLRequestClient {
    /// Notifies the client that the request has completed. Use the
    /// CefURLRequest::GetRequestStatus method to determine if the request was
    /// successful or not.
    func onRequestComplete(request: URLRequest)
    
    /// Notifies the client of upload progress. |current| denotes the number of
    /// bytes sent so far and |total| is the total size of uploading data (or -1 if
    /// chunked upload is enabled). This method will only be called if the
    /// UR_FLAG_REPORT_UPLOAD_PROGRESS flag is set on the request.
    func onUploadProgress(request: URLRequest, sentCount: Int64, totalCount: Int64)
    
    /// Notifies the client of download progress. |current| denotes the number of
    /// bytes received up to the call and |total| is the expected total size of the
    /// response (or -1 if not determined).
    func onDownloadProgress(request: URLRequest, receivedCount: Int64, totalCount: Int64)
    
    /// Called when some part of the response is read. |data| contains the current
    /// bytes received since the last call. This method will not be called if the
    /// UR_FLAG_NO_DOWNLOAD_DATA flag is set on the request.
    func onDownloadData(request: URLRequest, chunk: NSData)
...
public extension URLRequestClient {
    func onRequestComplete(request: URLRequest) {
    }
