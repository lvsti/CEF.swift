...
/// indicated.
/// CEF name: `CefFrameHandler`
public protocol CEFFrameHandler {

    /// Called when a new frame is created. This will be the first notification
    /// that references |frame|. Any commands that require transport to the
    /// associated renderer process (LoadRequest, SendProcessMessage, GetSource,
    /// etc.) will be queued until OnFrameAttached is called for |frame|.
    /// CEF name: `OnFrameCreated`
    ...
    /// CefLifeSpanHandler::OnBeforeClose() during browser destruction then
    /// CefBrowser::IsValid() will return false for |browser|.
    /// CEF name: `OnMainFrameChanged`
    func onMainFrameChanged(browser: CEFBrowser, oldFrame: CEFFrame?, newFrame: CEFFrame?)
}

public extension CEFFrameHandler {
    func onFrameCreated(browser: CEFBrowser, frame: CEFFrame) {
    }
