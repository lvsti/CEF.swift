...
public typealias CEFResolveCallbackOnResolveCompletedBlock = (results: [String]?, errorCode: CEFErrorCode) -> Void

class CEFResolveCallbackBridge: CEFResolveCallback {
    let block: CEFResolveCallbackOnResolveCompletedBlock
    
    init(block: CEFResolveCallbackOnResolveCompletedBlock) {
        self.block = block
    }
    
    func onResolveCompleted(results: [String]?, errorCode: CEFErrorCode) {
        block(results: results, errorCode: errorCode)
    }
}
